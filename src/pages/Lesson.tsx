      ...
      <PageSEO
        ...
        jsonLd={{
          ...
          "isPartOf": {
            "@type": "Course",
            "name": "學會分辨詐騙連結",
            "description": "七篇簡易教學帶你學會看懂網址門牌、分辨詐騙連結。",
          },
        }}
      />
      <LessonNav />
...
const Lesson1 = ({ onQuizDone }: { onQuizDone: () => void }) => (
  <div>
    ...
    <p className="mt-4">
      詐騙網站之所以能騙到人，就是因為我們常常忘了仔細檢查網址。詐騙網站會把網址做得很像真的，讓你一不注意就點進去。
    </p>
    <p className="mt-4">
      但只要你知道門牌在哪裡、怎麼看，就已經比大多數人更難被騙了。
    </p>
...
const Lesson7 = ({ onQuizDone }: { onQuizDone: () => void }) => (
  <div>
    ...
    <p className="mt-4">在電子郵件地址中，<strong>@ 後面才是門牌</strong>。而 @ 前面的名稱，任何人都可以隨便取——就像你申請 Gmail 時可以自己選帳號名稱一樣。</p>
    <p className="mt-4">同理，請注意<strong>寄件者的「名稱」也是可以自己設定的</strong>，所以就算顯示名稱寫「蝦皮購物」或「國泰世華銀行」，也不代表真的是他們寄的。你要看的是 @ 後面的門牌。</p>
    ...
    <KeyTakeaway>收到電子郵件時，不要相信寄件者名稱，找到電子郵件地址，直接看 @ 後面的門牌。門牌不對，就不要相信。</KeyTakeaway>
